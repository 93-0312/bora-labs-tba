<template>
  <div
    class="overflow-hidden relative rounded-lg border border-base-200 transition translate-y-0 will-change-transform hover:-translate-y-1 hover:shadow-[0_5px_15px] hover:shadow-neutral/20"
  >
    <a :href="`/tba/${props.ercType + '/' + props.id}`" class="block w-full p-2 md:p-3">
    ...
    <template v-if="props.hasBadge">
      ...
      <div
        ...
        class="absolute left-6 top-6 badge h-8 bg-neutral-content/60 backdrop-blur-md rounded-md border-none text-xs md:text-sm"
      >
        <!-- prettier-ignore -->
    ...
    <div class="px-2 md:px-3">
      ...
      <p
        ...
        :class="[
          ...
          'items-center',
          'font-medium',
          props.isSmall ? 'px-1 text-xs md:text-md' : 'h-8 px-1 text-sm md:h-10 md:px-2 md:text-base'
        ]"
      >
        {{ truncate(props.walletAddress) }}
        <button
          class="p-2 hover:text-neutral-focus"
          type="button"
          aria-label="copy"
          @click="copy(props.walletAddress)"
        >
          <!-- prettier-ignore -->
      ...
      <p
        ...
        :class="[
          'truncate',
          'font-bold',
          props.isSmall ? 'px-1.5 text-xs md:text-md' : 'h-8 px-1 text-sm leading-8 md:h-10 md:px-2 md:text-[20px] md:leading-10'
        ]"
      >
        {{ props.cardName }}
      </p>
      ...
      <p
        ...
        :class="[
          ...
          'text-neutral/60',
          'font-medium',
          props.isSmall ? 'p-1 text-[10px] md:text-md' : 'px-1 text-xs md:px-2 md:text-sm'
        ]"
      >
        Bora Testnet
      </p>
