<template>
  <button class="btn btn-sm btn-primary md:btn-md" v-if="!signed" @click="signin">
    Sign in
  </button>
  ...
  <template v-else>
    <div class="flex items-center pl-4 pr-1 border border-neutral-50 rounded-full">
      <div class="dropdown dropdown-bottom dropdown-end">
        <label tabIndex={0} class="flex items-center h-9 ic-metamask text-sm cursor-pointer md:h-12 md:text-base">
          0x3e5c...9981Cb
        </label>

        <button tabIndex={0} class="dropdown-content z-[1] px-4 py-3 -mr-[37px] mt-2 border border-gray-500 bg-gray-800 bg-opacity-50 backdrop-blur-md rounded-md text-xs shadow-sm hover:text-neutral-300" @click="signed = false">
          Sign Out
        </button>
      </div>
      
      <button class="p-2 hover:text-neutral-300">
